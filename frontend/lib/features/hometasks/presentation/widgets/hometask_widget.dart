...
class _HometaskWidgetState extends State<HometaskWidget> {
  ...
  void _saveEdit() {
    ...
    final normalizedItems = _editingItems.map((item) {
      ...
      if (isProgress) {
        return ChecklistItem(text: text, isDone: false, progress: item.progress ?? 0);
      }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if ((widget.hometask.description ?? '').trim().isNotEmpty)
              Padding(
            ...
            if (widget.onEditHometask != null)
              Align(
                ...
                child: TextButton.icon(
                  ...
                  icon: const Icon(Icons.edit),
                  label: Text(AppLocalizations.of(context)?.commonEdit ?? 'Edit'),
                ),
              ),
                          ...
                          items: [0, 1, 2, 3, 4]
                              .map(
                                (value) => DropdownMenuItem(
                                  value: value,
                                  child: Text(_progressLabel(context, value)),
                                ),
                              )
                              .toList(growable: false),
                          onChanged: (value) {
        ...
        children: [
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        hintText:
                            AppLocalizations.of(context)?.hometasksItemHint(index + 1) ??
                                'Item ${index + 1}',
          ...
          Align(
            ...
            child: TextButton.icon(
              ...
              label: Text(AppLocalizations.of(context)?.hometasksAddItem ?? 'Add item'),
            ),
          ),
          Padding(
            padding: const EdgeInsets.only(top: 12),
  ...
  String _progressLabel(BuildContext context, int progress) {
    final l10n = AppLocalizations.of(context);
    switch (progress) {
      case 0:
        return l10n?.hometasksProgressNotStarted ?? 'Not started';
      case 1:
        return l10n?.hometasksProgressInProgress ?? 'In progress';
      case 2:
        return l10n?.hometasksProgressNearlyDone ?? 'Nearly done';
      case 3:
        return l10n?.hometasksProgressAlmostComplete ?? 'Almost complete';
      case 4:
        return l10n?.hometasksProgressComplete ?? 'Complete';
      default:
        return 'Unknown';
    }
  }
